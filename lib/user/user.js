...
User.prototype.setNewGatherElo = async function (elo) {
    try {
        const profile = await Profile.findOneAndUpdate({ userId: this.id }, { $set: { gatherElo: elo } });
        profile.save((error) => {
...
User.findV2 = async function (id) {
    try {
        const body = await enslClient.getUserByIdV2(id);
        if (!body) return new Error("Unable to auth user against API");
        let user = new User(body);
        const tasks = [
            // Retrieve or create user profile from local store
            async () => {
                try {
                    const profile = await Profile.findOrCreatePromise(user);
                    user.profile = profile.toJson();
                    return profile;
                } catch (err) {
                    return new Error("An error occured while fetching stored profile of the user : " + id)
                }
            },
            // Retrive Hive Stats
            async () => {
                try {
                    const userStats = await hiveClient.getUserStatsPromise(user);
                    _.assign(user.hive, userStats);
                   return userStats;
                } catch (err) {
                    return new Error("An error occured while fetching hive stats of the user : " + id)
                }
            },
        ];
